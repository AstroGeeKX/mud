...
class AbstractRobot {
   public:
    ...
    // 是否能飞
    virtual bool canFlying() { return false; }
    // 是否是自动控制
...
class Pacifist : public AbstractRobot {
   public:
    // 武器
    void weapon() override { cout << "可以发射镭射光..." << endl; }
    // 外观
    void appearance() override {
        cout << "外部和巴索罗米·熊一样, "
                "体型庞大，拥有呈半圆形的耳朵，内部似乎金属。"
             << endl;
    }
    // 能力
    void fightAbility() override {
        cout << "结实抗揍, 可以通过手部或者嘴部发射镭射激光, 可以融化钢铁!!!"
             << endl;
    }
    string getName() override { return "和平主义者"; }
};
...
class Seraphim : public AbstractRobot {
   public:
    ...
    void weapon() override {
        cout << "可以发射镭射激光, 鹰眼外形的炽天使携带者一把巨剑, "
                "可以斩断一切!!!"
             << endl;
    }
    // 外观
    void appearance() override {
        cout << "外观和七武海小时候的外形一样, 并且拥有一对和烬一样的翅膀!!!"
             << endl;
    }
    // 能力
    void fightAbility() override {
        cout << "不仅可以发射镭射激光, 还拥有七武海的能力, 牛逼plus, 无敌了!!!!"
             << endl;
    }
    // 自愈能力
    ...
    // 是否能飞
    bool canFlying() override { return true; }
    string getName() override { return "炽天使"; }
};
...
// 作者: 苏丙榅
// 链接:
// https://subingwen.cn/design-patterns/template-method/#%E7%82%BD%E5%A4%A9%E4%BD%BF
// 来源: 爱编程的大丙
