/*
 *copy版本是阅读代码副本，随意修改，不是重要内容
 *epoll基于非阻塞I/O事件驱动
 *黑马程序员视频的代码，跟epoll_reactor2.c基本没什么区别，估计那个文件是抄写这里的
 */
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
void eventadd(int efd, int events, struct myevent_s *ev) {
    struct epoll_event epv = {0, {0}};  // 定义并初始化
    int op;
    epv.data.ptr = ev;                 // 挂载到ev上，实现mev放到红黑树上
    epv.events = ev->events = events;  // EPOLLIN 或 EPOLLOUT  监听事件类型

    if (ev->status == 0) {   // 已经在红黑树 g_efd 里 防御性编程
        op = EPOLL_CTL_ADD;  // 将其加入红黑树 g_efd, 并将status置1
        ev->status = 1;      // 应该把这个放到下边
    }
...
void eventdel(int efd, struct myevent_s *ev) {
    ...
    if (ev->status != 1)  // 不在红黑树上   就不用删除 防御性编程
        return;

    // epv.data.ptr = ev;
    epv.data.ptr = NULL;  // 不明意义
    ev->status = 0;       // 修改状态 第二个有用的东西
    epoll_ctl(efd, EPOLL_CTL_DEL, ev->fd,
              &epv);  // 从红黑树 efd 上将 ev->fd 摘除
    // 这是本函数实际有用的语句，其中ev传参为了得到ev->fd，从epoll监听列表上移除
    return;
}
...
void acceptconn(int lfd, int events, void *arg) {
    ...
    do {
        ...
        eventadd(g_efd, EPOLLIN,
                 &g_events[i]);  // 将cfd添加到红黑树g_efd中,监听读事件
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 2) port = atoi(argv[1]);  // 使用用户指定端口.如未指定,用默认端口
    ...
    while (1) {
        ...
        for (i = 0; i < 100;
             i++, checkpos++) {  // 一次循环检测100个。 使用checkpos控制检测对象
            ...
            if (duration >= 60) {
                ...
                eventdel(g_efd,
                         &g_events[checkpos]);  // 将该客户端 从红黑树 g_efd移除
            }
        }
